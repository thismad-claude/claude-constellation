...
// Global token stats (cumulative since server start)
let globalTokens = {
  totalInput: 0,       // New tokens sent to Claude
  totalOutput: 0,      // Tokens generated by Claude
  totalCacheCreation: 0 // Tokens added to cache
};
...
function processEvent(event) {
  ...
  // Process token data if provided
  // Hook sends LAST usage values from the most recent API call
  if (event.tokens && typeof event.tokens === 'object') {
    const session = state.sessions.get(sessionId);
    const t = event.tokens;

    // Last usage from this API call (for context calculation)
    const lastUsage = {
    ...
    // Add output tokens to cumulative total (what was generated this call)
    // For input/cache, we only count the NEW tokens (not re-reading cache)
    session.tokens.input += lastUsage.input;
    session.tokens.output += lastUsage.output;
    // cacheCreation = new tokens added to cache this call
    session.tokens.cacheCreation += lastUsage.cacheCreation;
    // Don't accumulate cacheRead - it's the same context being re-read

    // Update global tokens
    globalTokens.totalInput += lastUsage.input;
    globalTokens.totalOutput += lastUsage.output;
    globalTokens.totalCacheCreation += lastUsage.cacheCreation;

    // Calculate REAL context usage (input + cache_read from LAST call = what Claude sees now)
    const MAX_CONTEXT = 200000;
    const currentContext = lastUsage.input + lastUsage.cacheRead;
    ...
    broadcast({
      type: 'token_update',
      sessionId,
      tokens: session.tokens,           // Cumulative tokens for display
      globalTokens,
      lastUsage,                         // Last call values for animations
      contextUsage: session.contextUsage // Real context %
    });
  }
